...
function partitionWithRandomPivot(array, startIndex, endIndex) {
    ...
    // and swap it with the last item so that pivotIndex is somewhere in middle/random
    swap(array, randomIndex, endIndex);
    // take the num at random index as the pivot
    const pivot = array[endIndex];
    // console.log('pivot', pivot);
    // start with assuming startIndex is pivotPosition assuming all elements to right in sub-arr are greater than pivot
...
function quickSort(array, start, end) {
    ...
    if (start < end) {
        let pivotIndex = partitionWithRandomPivot(array, start, end);
