...
function largestProductinaSeries(n) {
    ...
    let thousandDigits2  = [7, 3, 1, 6, 8, 0, 4, 3, 6, 8, 7, 9, 5, 0, 6, 5, 0, 0, 0, 5, 9, 9, 0];
    let largestProduct  = 0;
    let windowStart     = 0;
    let windowEnd       = 0;
    let product         = 1;
    while (windowEnd <= thousandDigits.length - 1) {
        if ((windowEnd - windowStart + 1) < n) {
            product *= thousandDigits[windowEnd++];
            continue;
        } else {
            product *= thousandDigits[windowEnd];
            ...
            if (thousandDigits[windowStart] !== 0) {
                product /= thousandDigits[windowStart];
            } else {
                product    = 1;
                let subArr = windowStart + 1;
                while (subArr <= windowEnd) {
                    product *= thousandDigits[subArr++];
                }
            }
            windowStart++;
            windowEnd++;
...
Logger(1, largestProductinaSeries(4));
Logger(2, largestProductinaSeries(13));